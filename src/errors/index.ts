...
function logWarnings(errors: Errors): void {
  if (errors.skippedBecauseNotInResourceModules.length > 0) {
    ...
    console.error(
      errors.skippedBecauseNotInResourceModules
        .map(fileErrors => "  " + fileErrors)
        .join("\n")
    );
  }
}
...
function hasErrors(fileErrors: FileErrors): boolean {
  return Object.keys(fileErrors).some(
    (k: string): boolean =>
...
function logMissingMessages(file: string, fileErrors: FileErrors) {
  if (fileErrors.missingMessages && fileErrors.missingMessages.length > 0) {
    ...
    const messagesByModule: MessagesByModule = fileErrors.missingMessages.reduce(
      (
        acc: MessagesByModule,
        { message, modules }: MissingMessage
      ): MessagesByModule => {
    ...
    fileErrors.missingTemplates.forEach(({ kind, template, modules }) => {
      switch (kind) {
        ...
        case "template_not_in_dependencies":
          ...
          console.error(
            `  Template not found in dependencies: ${template.module} ${
              template.fileName
            }`
          );
          console.error(
            `    Not found in dependencies of modules: ${modules.join(", ")}`
          );
          break;
      }
    });
...
function logUnusedDefines(file: string, fileErrors: FileErrors) {
  if (fileErrors.unusedDefines && fileErrors.unusedDefines.length > 0) {
    ...
    console.error(fileErrors.unusedDefines.map(name => `  ${name}`).join("\n"));
  }
}
    ...
    fileErrors.dependencies.forEach(error => {
      switch (error.kind) {
        case "multiple_defines":
          ...
          console.error(
            error.where.map(([fileErrors]) => `    ${fileErrors}`).join("\n")
          );
          break;
        case "not_defined":
          console.error(
            `  Required ${error.id} not defined in any source files`
          );
          break;
        case "file_in_multiple_dependencies":
          console.error(
            `  Required ${error.id} defined in file ${
              error.where[0]
            } found in multiple ResourceModules:`
          );
