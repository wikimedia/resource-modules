...
export function getDependenciesWithFile(
  scriptToFind: string,
  moduleName: string,
  module: ResourceModule | null | undefined,
    ...
    if (Array.isArray(module.dependencies)) {
      ...
      found = found.concat(
        getDependenciesWithFile(
          scriptToFind,
          module.dependencies,
          resourceModules[module.dependencies],
          resourceModules
        )
      );
    }
  ...
  if (module.messages) {
    if (
      ...
      (module.messages.constructor === Object &&
        Object.keys(module.messages).some(
          (weirdKey: string): boolean =>
            weirdKey === msgToFind ||
            (<any>module.messages)[weirdKey] === msgToFind
        ))
    ) {
      found.push(moduleName);
    }
  ...
  if (module && module.dependencies) {
    if (Array.isArray(module.dependencies)) {
      ...
      module.dependencies.forEach((dep: string) => {
        found = found.concat(
          getDependenciesWithModule(moduleToFind, dep, resourceModules)
        );
      });
    } else if (
