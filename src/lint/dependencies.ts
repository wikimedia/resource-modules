...
export default function getDependenciesErrors(
  ...
  if (ana.requires && ana.requires.length) {
    return ana.requires.reduce(
      (errs: DependencyError[], mfId: string): DependencyError[] => {
        ...
        const whoDefines: FileAndAnalysis[] = Object.keys(analysis.files)
          ...
          .filter(
            ([f, fa]: FileAndAnalysis): boolean =>
              fa.defines &&
              Array.isArray(fa.defines) &&
        ...
        if (whoDefines.length > 1) {
          errs.push({
            ...
            id: mfId,
            where: whoDefines.sort(([f1, _], [f2, __]) => (f1 > f2 ? 1 : -1))
          });
